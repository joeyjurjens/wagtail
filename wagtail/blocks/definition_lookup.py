...
class BlockDefinitionLookup:
    ...
    def __init__(self, blocks):
        self.blocks = blocks
        self.block_classes = {}
        # The set of indexes currently being constructed (the construction stack). A
        # child that references an index already on the stack is a back-edge that would
        # close a cycle; get_block hands back a lazy BlockReference for it rather than
        # recursing, keeping construction finite. Wagtail builds a fresh block instance
        # per reference (cyclic or not), so the reference resolves to a fresh instance on
        # access and traversal terminates via Block._definition_id, never via shared
        # identity.
        self._constructing = set()

    def get_block(self, index):
        if index in self._constructing:
            # Back-edge to an ancestor still being constructed: defer to a lazy reference
            # so we don't recurse forever. It resolves to a fresh block when accessed.
            return BlockReference(lambda i=index: self.get_block(i))

        self._constructing.add(index)
        try:
            path, args, kwargs = self.blocks[index]
            block = self._load_class(path).construct_from_lookup(self, *args, **kwargs)
        finally:
            self._constructing.discard(index)

        # Give the block the node identity of its lookup index, so the cycle guard in
        # guard_full_graph_method (and the builder) can detect re-entry across fresh
        # re-materialisations (different id(), same index). See Block._definition_id.
...
class BlockDefinitionLookupBuilder:
    ...
    def __init__(self):
        ...
        # Index of each block we have fully added, keyed by node identity
        # (block._definition_id), so the same node re-used in several places is only
        # stored once. Node identity (not id()) is required here because a block rebuilt
        # from a cyclic lookup yields a *fresh* instance on every reference resolution —
        # those fresh instances share a _definition_id but not an id(), and an id()-keyed
        # builder would fail to recognise the cycle and recurse forever.
        self.block_indexes_by_identity = {}

        # Nodes whose deconstruction is in progress, keyed by _definition_id. The value is
        # the slot reserved for the block (or None if nothing has needed it yet). Used to
        # break cyclic graphs: a back-reference to a node still being deconstructed reserves
        # its slot here instead of recursing forever.
