...
class SelfRefStructBlockMigrationTest(TestCase):
    """
    Block migration utilities must handle a self-referential cyclic block.
    SelfRefStructBlock.selfref is a ListBlock whose child is a BlockReference
    back to SelfRefStructBlock itself.
    """
    ...
    def test_blocks_and_data_not_operated_on_intact(self):
        ...
        # Top-level char1 of the selfrefstruct block unchanged (path targets children only)
...
class MutualStructBlockMigrationTest(TestCase):
    """
    Block migration utilities must handle a mutually-referential cyclic block.
    MutualStructBlock.stream1 is a BlockReference to MutualStreamBlock, which
    in turn contains MutualStructBlock as a child.
    """
    ...
    def test_blocks_and_data_not_operated_on_intact(self):
        ...
        self.assertEqual(altered[1], self.raw_data[1])
        # Outer char1 of mutualstruct unchanged
        self.assertEqual(altered[0]["value"]["char1"], "Outer")
        self.assertEqual(altered[0]["id"], "a")
        # Non-struct1 child (char1) unchanged
        self.assertEqual(
